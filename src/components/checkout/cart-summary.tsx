...
export function CartSummary({ items, onUpdateQuantity, onRemoveItem, isEditable = true }: CartSummaryProps) {
  ...
  return (
    <div className="bg-brand-surface-alt/50 rounded-2xl p-6 text-foreground">
      ...
      <div className="space-y-4 mb-6">
        {items.map((item) => (
          <div key={`${item.id}-${item.size}-${item.color}`} className="flex gap-4">
            <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-brand-surface" style={{ position: 'relative' }}>
      ...
      <div className="border-t border-border pt-4 space-y-2">
        ...
        {shipping === 0 && <p className="text-xs text-green-600">¡Envío gratis por compras superiores a $50.000!</p>}
