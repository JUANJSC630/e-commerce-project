...
/** Normalizes the configured value into a wa.me URL (accepts a raw number or a full URL). */
function toWhatsAppUrl(value: string, message: string): string | null {
  const trimmed = value.trim()
  if (!trimmed) return null
  const base = /^https?:\/\//i.test(trimmed)
    ? trimmed
    : `https://wa.me/${trimmed.replace(/[^\d]/g, "")}`
  if (base === "https://wa.me/") return null
  const sep = base.includes("?") ? "&" : "?"
  return `${base}${sep}text=${encodeURIComponent(message)}`
}

/**
 * Floating WhatsApp contact button (fixed bottom-right). Reads the number from
 * the store's social settings and renders nothing when it isn't configured, so
 * the merchant turns it on purely from the admin.
 */
